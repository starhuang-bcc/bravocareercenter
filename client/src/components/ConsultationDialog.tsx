...
export function ConsultationDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={closeDialog}>
      ...
      <DialogContent className="max-h-[92vh] overflow-y-auto border-[#dbe4f0] bg-[#fbfcfe] p-5 sm:max-w-xl sm:p-7">
        {submitted ? (
          ...
          <>
            <DialogHeader className="pr-8">
              ...
              <DialogDescription className="text-left text-base leading-7 text-[#606975]">
                請選擇服務、方式與方便時段，送出後將由團隊安排諮詢。
              </DialogDescription>
            </DialogHeader>
            ...
            <form
              ...
              onSubmit={(event) => {
                ...
                submit.mutate({ ...form, message: form.message || undefined });
              }}
            >
              <fieldset className="space-y-2">
                <legend className="text-sm font-semibold text-[#172033]">選擇諮詢服務</legend>
                <div className="grid gap-2 sm:grid-cols-2">
                  {(["career", "dream"] as const).map((type) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-xl border px-4 py-3 text-left text-sm font-semibold transition-[border-color,background-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#185abc] ${
                          selected
                            ? "border-[#2563c7] bg-[#eef5ff] text-[#1f5db6] shadow-sm"
                            : "border-[#dbe2ea] bg-white text-[#4e5968] hover:border-[#b8c8dc]"
                        }`}
                      >
                        {type === "career" ? "職涯諮詢" : "築夢諮詢"}
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="rounded-xl bg-[#f1f5fa] p-4 text-sm leading-6 text-[#5e6978]">
                <p><strong className="text-[#172033]">諮詢時間：</strong>60 分鐘</p>
                <p><strong className="text-[#172033]">諮詢費用：</strong>線上 $800／次；現場 $1,000／次</p>
                <p><strong className="text-[#172033]">付款方式：</strong>{form.consultationMode === "online" ? "諮詢後付款" : "諮詢後現場付款"}</p>
              </div>

              <fieldset className="space-y-2">
                <legend className="text-sm font-semibold text-[#172033]">諮詢方式</legend>
